	...
	if($this->input->post('Idnum') !== NULL){
		...
		if($p->checkInstructor($idNum, $pass) >= 1){	
			...
			$data = $p->getInstructorInfo($idNum);
                ...
                'Fname' => $data['Fname'],
				'Vacant_Sched' => $data['Vacant_Sched'],
                'Lname' => $data['Lname'],
                'Gender' => $data['Gender'],
...
$data=array();
$data=$p->getOneviewC();
  $this->load->view('viewCourseInfo',$data);  
        $this->load->view('footer');
  }
  ...
  $this->load->view('newUser');
      ...
      public function InstructorTable() {
		...
		if($this->input->get('id') !== NULL)
			$data['inst_id']= $this->input->get('id');
...
$data=$p->getOneviewA();
  ...
  public function reactivate_inst()
  {
	  
		$this->load->model('InstructorModel');
		$p = new InstructorModel();
	  
		$p->reactivate_inst($this->input->get('Idnum'));
		
		redirect('Admin/InstructorTable?inst_id='.$this->input->get('Idnum'),'refresh');
  ...
  public function reactivate()
  {
		...
		//* print_r($this->input->post('instructors')[0]);


		
		for($i=0; !empty($this->input->post('instructors')[$i]); $i++){
			
			//$upd_course['courseID']= $this->input->post('course_id')[$i];
			
			//* $upd_course['date_expired']= $this->input->post('instructors')[$i];

			$upd_course['IdNum']= $this->input->post('instructors')[$i];
			
			
			$result= $p->reactive_courses($upd_course, $this->input->get('IdNum'));
			...
			$result= $p->archiveInstructor($this->input->get('IdNum'));
...
public function UpdateCourse_Proc() {
...
$p->courseDescription=$this->input->post('courseDescription');
$p->category=$this->input->post('category');
$p->Idnum= $this->input->post('inst_id');
$p->courseStatus=$this->input->post('courseStatus');
$p->courseType=$this->input->post('courseType');
$p->price=($this->input->post('courseType') == 'free')? 0: $this->input->post('price');
...
redirect('Admin/loadcourses?course_id='.($p->courseID),'refresh');
}
}
...
public function addStuinCourse() {
	...
	$p->course_id=$this->input->post('course_id');
	$p->Email=$this->input->post('Email');
	$p->username=$p->getStudInfo($this->input->post('Email'))['username'];
	$p->flag=$this->input->post('flag');
	$r=array();
	$r=$p->selectstuCourse();
...
redirect('Admin/enrollStud','refresh');
}
}
